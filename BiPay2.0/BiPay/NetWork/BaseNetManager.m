...
//nohost
+(void)ylNonTokenRequestWithPostNoHost:(NSString *)urlString parameters:(NSDictionary *)parameter successBlock:(ResultBlock)resultBlock{
    NSMutableDictionary *para = [parameter mutableCopy];
...
//post--
+(void)coinNameylNonTokenRequestWithGET:(NSString *)urlString parameters:(NSDictionary *)parameter  withCoinName:(NSString*)coinname successBlock:(ResultNameBlock)resultBlock{
    NSMutableDictionary *para = [parameter mutableCopy];
    NSString *urlStr =  [NSString stringWithFormat:@"%@%@",HOST,urlString];
    NSLog(@"接口&&参数%@%@",urlStr,para);
    NSMutableDictionary *dict=[NSMutableDictionary dictionaryWithCapacity:0];
    ...
    [BaseNetManager requestWithPost:urlStr header:dict parameters:para successBlock:^(id resultObject, int isSuccessed) {
        if (isSuccessed) {
            resultBlock(resultObject,1,coinname);
            // DLog(@"%@",resultObject);
        }else{
            resultBlock(nil,0,coinname);
        }
    }];
}
@end
